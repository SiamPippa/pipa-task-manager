...
class DailyReportController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $filters = $this->scopedFilters($request, ['search', 'company_id', 'user_id', 'project_id', 'task_id', 'date_from', 'date_to']);
        $filters['viewer_id'] = auth()->id();

        return view('daily-reports.index', [
            'dailyReports' => $this->dailyReportService->paginate($filters, 15, ['user', 'project', 'task', 'timeLog']),
            'filters' => $filters,
    ...
    {
        $dailyReport = $this->dailyReportService->findOrFail($daily_report, ['user', 'project', 'task', 'timeLog']);
