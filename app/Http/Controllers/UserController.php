...
class UserController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $filters = $this->scopedFilters($request, ['search', 'company_id', 'designation_id', 'role', 'status']);

        return view('users.index', [
            'users' => $this->userService->paginate($filters, 15, ['company', 'designation', 'officeLocation', 'roles']),
            'filters' => $filters,
    ...
    private function officeLocations()
    {
        return $this->scopedForCompany(OfficeLocation::query()->orderBy('name')->get());
    }
}
